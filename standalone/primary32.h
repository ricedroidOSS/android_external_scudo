...
private:
  ...
  NOINLINE uptr releaseToOSMaybe(SizeClassInfo *Sci, uptr ClassId,
                                 ReleaseToOS ReleaseType = ReleaseToOS::Normal) REQUIRES(Sci->Mutex) {
    ...
    const bool CheckDensity = BlockSize < PageSize / 16U && ReleaseType != ReleaseToOS::ForceAll;
    // Releasing smaller blocks is expensive, so we want to make sure that a
    // significant amount of bytes are free, and that there has been a good
    // amount of batches pushed to the freelist before attempting to release.
    if (CheckDensity) {
      if (ReleaseType == ReleaseToOS::Normal && BytesPushed < Sci->AllocatedUser / 16U)
        return 0;
    }
